...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/25 15:31:42 by rgarcia           #+#    #+#             */
/*   Updated: 2023/02/27 16:04:14 by warnora          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	var.line = NULL;
	var.flag = 0;
	var.eof = gnl_init(gnl, fd);
	while (var.eof > 0 && var.flag != 1)
	{
		var.i = ft_find_newline(gnl);
		if (var.i == BUFFER_SIZE)
		{
			var.line = ft_strjoin_to_i(var.line, gnl, var.i);
			var.eof = manage_buffer(gnl, &var.flag, fd);
		}
	...
	if ((var.eof == 0 && var.flag != 1) || var.eof < 0)
		return (NULL);
	var.eof = ft_bufcpy_from_i(gnl, var.i + 1, var.eof - 1);
	return (var.line);
}
